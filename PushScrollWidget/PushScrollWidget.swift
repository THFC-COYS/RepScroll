    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: StreakProvider()) { entry in
            StreakWidgetView(entry: entry)
                .containerBackground(PushScrollWidgetTheme.background, for: .widget)
        }
        .configurationDisplayName("PushScroll Streak")
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: "flame.fill")
                    .foregroundStyle(PushScrollWidgetTheme.accent)
                Text("PushScroll")
                    .font(.caption.weight(.bold))
                    .foregroundStyle(PushScrollWidgetTheme.textSecondary)
            }
            Text("\(entry.streak)")
                .font(.system(size: 44, weight: .bold, design: .rounded))
                .foregroundStyle(PushScrollWidgetTheme.textPrimary)
            Text("day streak")
                .font(.caption)
                .foregroundStyle(PushScrollWidgetTheme.textSecondary)
        }
...
@main
struct PushScrollWidgetBundle: WidgetBundle {
